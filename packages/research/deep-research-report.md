...
```mermaid
xychart-beta
    title "Diet-category associations with overweight/obesity (US vet practices, 1995): adjusted odds ratios"
    x-axis ["Semi-moist (Overweight)","Canned (Obese)","Homemade (Obese)","Other foods (Obese)"]
    y-axis "Odds ratio (vs popular dry)" 1.0 --> 2.5
    bar [1.5, 1.4, 1.9, 2.3]
```
...
```mermaid
flowchart TD
  A[Owner feeds store-bought food] --> B{Trigger?}
  B -->|Overweight / vet comment| C[Weight & BCS audit + MER explanation]
  B -->|GI/skin frustration| D[Ingredient control + recipe transparency]
  B -->|Trust/opacity concerns| E[Label limits + nutrient-per-1000kcal demo]
  C --> F[Trial kit: 14 days]
  D --> F
  E --> F
  F --> G[4-week check-in: weight/BCS + adjust]
  G --> H[Subscription tier: full kit or premix-only]
  H --> I[Quarterly outcomes report + vet-shareable summary]
```
