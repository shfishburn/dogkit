...
import imageSystemPrompt from "@data/prompts/canine_recipe_image_system_v1.txt?raw";
import imageUserTemplate from "@data/prompts/canine_recipe_image_user_v1.txt?raw";
...
export const POST: APIRoute = async ({ request }) => {
  ...
  try {
    ...
    const userPrompt = imageUserTemplate
      ...
      .replaceAll("{{veggie}}", reqBody.veggie)
      .replaceAll("{{cook_method}}", reqBody.cook_method)
    ...
    if (imageUrl) {
      ...
      await supabaseUpdate("recipes", reqBody.recipe_id, {
        image_url: imageUrl,
